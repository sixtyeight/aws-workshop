...
public class AwsS3Playground {
	...
	private final static String TEMP_DIR = "/home/m68k/wjax2017/aws-workshop/TEMP/";

	// TODO replace "YOUR_UPLOAD_FILE" with a file inside your temp directory of
	// your choice
	private final static String UPLOAD_FILE_NAME = "cat.jpg";

	// TODO make sure that "OWNER" is already set in project 00_common class
	// AwsAccount
	...
	/**
	 * Initializes playground and works through the different showcases. All
	 * showcases can be turned on/off via corresponding
	 * <code>PlaygroundToggle</code>
	 *
	 * <ul>
	 * <li>create bucket</li>
	...
	private void createBucket(String bucketName) {
		 ...
		 *
		 * TODO create bucket - use s3Client's createBucket(...) method to create bucket
	 ...
	 *
	 * @param bucketName
	 *            Name of the bucket to copy the file to
	 * @param fileName
	 *            Full qualified file name
	 * @param keyName
	 *            Key of the s3 object to create
	 */
	private void copyFileToBucket(String bucketName, String fileName, String keyName) {
		 ...
		 *
		 * TODO copy local file to bucket - use s3Client's putObject(...) method to copy
		 * file (you will need a RequestBody of a File instance) - log/print out objects
		 * eTag with the help of the response
		 */

		PutObjectRequest por = PutObjectRequest.builder().bucket(bucketName).key(keyName).build();
		s3Client.putObject(por, new File(fileName).toPath());
	}
	...
	private void listBuckets() {
		...
		/*
		 * TODO list all buckets of current account - use s3Client's listBuckets(...)
		 * method to list all buckets - log/print out bucket name and creation date for
		 * each bucket - call playground listObjects method for each bucket to retrieve
		 * buckets objects
		 */
	 ...
	 *
	 * @param bucketName
	 *            Bucket name to list objects for
	 */
	private void listObjects(String bucketName) {

		/*
		 * TODO list all objects of bucket with given name - use s3Client's
		 * listObjects(...) method to list all objects - log/print out objects key,
	 ...
	 *            Bucket name
	 * @param keyName
	 *            Unique key of object to copy
	 * @param filePath
	 *            Full qualified filename (including path informatio) to store the
	 *            object at
	 *
	 */
	private void copyFileFromBucket(String bucketName, String keyName, Path filePath) {
		 ...
		 *
		 *
		 * TODO copy local file to bucket - use s3Client's getObject(...) method to copy
		 * file from bucket to your local file system - log/print out objects available
		 * information (if any) with the help of the response
		 */

		GetObjectRequest gor = GetObjectRequest.builder().bucket(bucketName).key(keyName).build();
		s3Client.getObject(gor, filePath);
	}
	 ...
	 *
	 * @param bucketName
	 *            Name of bucket to delete all objects from
	 */
	private void deleteObjects(String bucketName) {
		...
		/*
		 *
		 * TODO delete all objects within bucket - use s3Client's listObjects(...)
		 * method to retrieve all objects within a bucket - use s3Client's
		 * deleteObject(...) method to delete specific file from bucket - log/print out
		 * name of delete objects and total number of deleted objects
		 */
	}
	...
	private void deleteBucket(String bucketName) {
		...
		/*
		 *
		 * TODO delete a single bucket - use s3Client's deleteBucket(...) method to
		 * delete specific bucket - log/print out corresponding response
		 */
	}
	...
	/**
	 * Helper method to generate a unique download file name with the help of the
	 * current "timestamp"
	 *
	 * @param fileName
	 *            non-unique name of the file
